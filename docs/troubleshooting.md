...
Most Orbit issues come down to the pane not being open, the active note, or an
...
### The Orbit pane won't open

- Confirm Orbit is enabled under **Settings → Community plugins**.
- Run **Orbit: Open** from the command palette — the pane opens in the right sidebar.
...
The note probably matches an exclusion. Check **Settings → Orbit → Advanced →
...
is on (Settings → Orbit → Relations).

### The 2nd-hop / Related list looks cut off

It's capped. Raise **Second-hop cap** (Settings → Orbit → Relations) to show more, or
...
1. Turn on **Settings → Orbit → Advanced → Debug logging**.
...
3. Reproduce the problem and look for lines prefixed with `[Orbit]`.
...
[github.com/MMoMM-org/obsidian-orbit/issues](https://github.com/MMoMM-org/obsidian-orbit/issues).

Please include:

- Your **Obsidian version** and **Orbit version** (Settings → Community plugins).
- Your **platform** (desktop OS or mobile).
- **Steps to reproduce** the problem.
- Any relevant **`[Orbit]` console output** captured with Debug logging on.
