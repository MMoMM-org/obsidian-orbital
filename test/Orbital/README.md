...
1. Open this directory as a vault in Obsidian (**Open folder as vault** → select `test/Orbital`). The directory name becomes the vault name in Obsidian's sidebar.
...
3. Run the build from the repo root:
   ...
   `manifest.json`) into `.obsidian/plugins/orbital/` and drops a
...
- `.obsidian/plugins/orbital/` — created and populated by the build (gitignored). Hot Reload watches it via the `.hotreload` marker.
